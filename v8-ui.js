...
function wx8Interp(a,b){return Number.isFinite(a)&&Number.isFinite(b)?(a+b)/2:(Number.isFinite(a)?a:b)}
function wx8WeatherIcon(code){if(code===0)return'☀️';if([1,2].includes(code))return'🌤️';if(code===3)return'☁️';if([45,48].includes(code))return'🌫️';if(code>=51&&code<=67)return'🌧️';if(code>=71&&code<=77)return'🌨️';if(code>=80&&code<=82)return'🌦️';if(code>=95)return'⛈️';return'⛅️'}
...
function wx8Nav(){
  ...
  if(window.locPrev)locPrev.onclick=()=>move(-1);if(window.locNext)locNext.onclick=()=>move(1);
}
nav=wx8Nav;
