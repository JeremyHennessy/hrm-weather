...
(()=>{
  ...
  function paint(){if(painting)return;painting=true;try{placeConfidence();placeHourly();observeOrder();const s=summary(),target=document.querySelector('#daySummary');if(s&&target){if(target.textContent!==s)target.textContent=s;target.dataset.source='engine3-summary';target.dataset.truth='live-current-hourly'}paintUV();changeNotice()}finally{painting=false}}
