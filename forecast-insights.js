...
(()=>{
  ...
  function paintUV(){
    ...
    if(!a){el.hidden=true;hero?.classList.remove('uvGuidanceActive');return}
    el.innerHTML=`<b>${a.title}</b><span>${a.detail}</span>`;el.hidden=false;hero?.classList.add('uvGuidanceActive');
  }
  function changeNotice(){
    ...
    let note=document.querySelector('#forecastChangeNotice');if(!changes.length){note?.remove();return}if(!note){note=document.createElement('div');note.id='forecastChangeNotice';note.className='forecastChangeNotice';document.querySelector('.dayBrief')?.prepend(note)}note.textContent=`Forecast changed: ${changes.join(' · ')}.`;
  }
  function paint(){const s=summary(),target=document.querySelector('#daySummary');if(s&&target){target.textContent=s;target.dataset.source='engine3-summary'}paintUV();changeNotice()}
  ...
  window.WXRefreshForecastInsights=paint;if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>setTimeout(paint,300),{once:true});else setTimeout(paint,300);window.addEventListener('wx-v3-ready',()=>setTimeout(paint,60));document.querySelector('#tabs')?.addEventListener('click',()=>setTimeout(paint,120));setInterval(paint,30000);
})();
