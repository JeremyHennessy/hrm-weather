(()=>{
  ...
  const humidex=(t,rh)=>{if(!Number.isFinite(t)||!Number.isFinite(rh)||t<18)return t;const a=17.27,b=237.7,alpha=(a*t)/(b+t)+Math.log(Math.max(.01,rh/100)),dew=(b*alpha)/(a-alpha),e=6.11*Math.exp(5417.7530*(1/273.16-1/(273.15+dew)));return t+(5/9)*(e-10)};
  ...
  function paint(engine,source='hourly'){
    ...
    const air=n(row.temperature_2m),rh=n(row.relative_humidity_2m),feel=humidex(air,rh),rain=n(row.precipitation_probability),unc=n(row.v2_uncertainty??row.uncertainty),labels=LOC_LABELS[key]||[key.toUpperCase(),key];
    if(!Number.isFinite(air))return false;
    put('kicker',labels[0]);put('place',labels[1]);put('feels',deg(feel));
    const actual=$('actual');if(actual)actual.innerHTML=`Actual <b>${deg(air)}</b>`;
    put('range',Number.isFinite(unc)?`Real Feel range ${deg(feel-unc)}–${deg(feel+unc)}`:'Real Feel from latest consensus');put('fhigh',deg(feel));put('rain',pct(rain));
    put('officialTemp',deg(air));put('officialStation','Weather Consensus collected data');put('obsline',`Loaded from ${source} data${engine?.updated_at?' · '+prettyAge(engine.updated_at):''}`);
    put('outside',feel>=31?'Hot outside':feel>=25?'Warm outside':feel>=10?'Comfortable outside':'Cool outside');put('advice','Forecast is ready; live sources are refining it in the background.');put('modelCount','cached consensus · live updating');put('uncertainty',Number.isFinite(unc)?`±${Math.round(unc*10)/10}°`:'--°');
    ...
    const rows=leadRows(c),hours=$('hours');if(hours&&rows.length)hours.innerHTML=rows.filter(([lead])=>lead<=12).map(([lead,x])=>`<div class="hour"><small>+${lead}H</small><div class="wx">⛅️</div><b>${deg(n(x.temperature_2m))}</b><div class="sub">Rain ${pct(n(x.precipitation_probability))}</div></div>`).join('');
    ...
    window.__wxInitialForecastShown=true;window.__wxStaticStartupShown=true;window.__wxStaticStartup={location:key,air,feel,rain,updated_at:engine?.updated_at||null,source};return true;
  }
