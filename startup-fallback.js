(()=>{
  ...
  async function fastCurrent(){
    const key=localKey(),points=CURRENT_POINTS[key]||CURRENT_POINTS.hrm,tz=timezone(key),ctrl=new AbortController(),timer=setTimeout(()=>ctrl.abort(),6000);window.__wxFastCurrent={location:key,painted:false,status:'loading',started_at:Date.now()};
    try{
      ...
      const settled=await Promise.allSettled(jobs),rows=settled.filter(x=>x.status==='fulfilled'&&x.value?.current&&n(x.value.current.apparent_temperature)!=null).map(x=>x.value);if(localKey()!==key)return false;
      const feel=avg(rows.map(x=>x.current.apparent_temperature)),air=avg(rows.map(x=>x.current.temperature_2m)),wind=avg(rows.map(x=>x.current.wind_speed_10m)),gust=avg(rows.map(x=>x.current.wind_gusts_10m));if(!Number.isFinite(feel))throw Error('current apparent temperature unavailable');
      ...
      const feels=$('feels');if(feels){feels.textContent=`${feel.toFixed(1)}°`;feels.dataset.currentSource='provider-apparent-fast-current';delete feels.dataset.engine3RealFeel}const actual=$('actual');if(actual&&Number.isFinite(air))actual.innerHTML=`Actual <b>${air.toFixed(1)}°</b>`;
      put('range','Current Real Feel · live inputs');if(Number.isFinite(wind))put('wind',`${Math.round(wind)} / ${Number.isFinite(gust)?Math.round(gust):'--'}`);put('obsline',`Live current conditions · ${rows.length}/${points.length} core point${points.length===1?'':'s'}; official observation correction updating`);document.documentElement.dataset.wxRealFeel='live-current-provider-apparent';
      window.__wxFastCurrent={location:key,painted:true,status:'ready',source:'provider-apparent-current',timezone:tz,feel,air,points:rows.length,total_points:points.length,point_values,elapsed_ms:Date.now()-(window.__wxFastCurrent?.started_at||Date.now())};window.dispatchEvent(new CustomEvent('wx-fast-current-ready',{detail:window.__wxFastCurrent}));return true;
