...
(()=>{
  ...
  function render(){
    ...
    const mos=d.mos?.['6']||{},analog=d.analogs?.['6']||{},rain=d.precip_calibration?.['6']||{},nudge=h.components?.observation_nudge,rf=h.real_feel_engine||{};
    const wf=V3.walk_forward_verification?.locations?.[locKey()]?.['6']?.scores?.v3_reconstructed||{};
    ...
    set('v3RealFeel',rf.available?`${rf.local_correction?.samples||0} samples`:'learning');set('v3Walk',wf.n?`${wf.mae?.toFixed?.(2)??'--'}° MAE`:'learning');
    ...
    const note=document.getElementById('v2EngineNote');if(note)note.textContent=`V3: local MOS + historical analogs + observation nudging + calibrated Real Feel + calibrated rain · ${V3.collector?.verified_ledger_rows??0} verified ledger rows.`;
  }
