...
try{
  ...
  await page.waitForFunction(()=>document.querySelector('#daySummary')?.dataset?.source==='live-current-hourly-summary',{timeout:10000});

  const before=await page.evaluate(()=>({fast:window.__wxFastCurrent,summary:document.querySelector('#daySummary')?.textContent||''}));
  ...
  const result=await page.evaluate(()=>{
    ...
    return{summary:document.querySelector('#daySummary')?.textContent||'',source:document.querySelector('#daySummary')?.dataset?.source||'',fast:window.__wxFastCurrent};
  });
  if(result.source!=='live-current-hourly-summary')throw Error(`wrong summary owner: ${result.source}`);
