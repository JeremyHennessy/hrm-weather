...
try{
  ...
  const state=await page.evaluate(()=>{
    ...
    // Add daytime evidence for the daily cards. Tomorrow is dry/cloudy; day 3
    // contains a wet code and must keep its existing rain icon/ownership.
    ...
    // A later wet current signal must remove cloud ownership rather than leaving
    // stale sunny/cloudy metadata on the persistent hero element.
