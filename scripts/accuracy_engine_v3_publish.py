...
def apply_adaptive_verification(engine:dict,state:dict,walk:dict)->None:
    """Reblend V3 only after champion/challenger promotion checks."""
    summary={};gate_summary={}
    for loc,payload in (engine.get("consensus") or {}).items():
        regime=((payload.get("regime") or {}).get("name")) or "unknown";loc_summary={};loc_gates={}
        for lead_s,h in (payload.get("hours") or {}).items():
            ...
            weighted=[]
            if v2temp is not None:weighted.append((v2temp,0.58))
            if mos is not None:
                base=0.27 if int(h.get("mos_samples",0))>=24 else 0.18;weighted.append((mos,base*float(mos_skill["factor"])))
            if analog is not None:weighted.append((analog,0.15*float(analog_skill["factor"])))
            if weighted:
            ...
            h["adaptive_skill"]={"mos":mos_skill,"analog":analog_skill,"observation_nudge":nudge_skill,"precipitation_calibration":pop_skill}
            loc_summary[lead_s]=h["adaptive_skill"];loc_gates[lead_s]={"mos":mg,"analog":ag,"observation_nudge":ng}
        summary[loc]=loc_summary;gate_summary[loc]=loc_gates
...
def main()->None:
    v2_engine=core.load(core.ENGINE,{});ledger=core.load(core.LEDGER,[]);skill=core.load(core.SKILL,{});observations=skill.get("observations",{});forecasts={name:{} for name in core.LOCATIONS}
    verification=verify.load_state();shadow_scored=verify.score_due(verification,observations)
    jobs=[]
    ...
    walk=walkforward.build(ledger);engine["walk_forward_verification"]=walk
    ...
    print(f"accuracy-v3 forecasts={engine['collector']['deterministic_forecasts']} verified={engine['collector']['verified_ledger_rows']} mos_ready={mos_ready} analog_ready={analog_ready} realfeel_ready={engine.get('real_feel',{}).get('forecast_points_ready',0)} shadow_scored={shadow_scored} shadow_added={shadow_added}")
