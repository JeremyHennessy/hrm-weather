...
try{
  ...
  catch{dataMode='node-live-fallback';stage='fallback-weather';try{const q=new URLSearchParams({latitude:'44.6822',longitude:'-63.6012',timezone:'America/Halifax',current:'temperature_2m,apparent_temperature,relative_humidity_2m,wind_speed_10m,wind_gusts_10m,weather_code',hourly:'precipitation_probability,uv_index',forecast_days:'1',temperature_unit:'celsius',wind_speed_unit:'kmh'});const r=await fetch('https://api.open-meteo.com/v1/forecast?'+q,{signal:AbortSignal.timeout(8000)});if(!r.ok)throw new Error('Open-Meteo '+r.status);const j=await r.json();const i=Math.max(0,(j.hourly?.time||[]).findIndex(t=>t.slice(0,13)>=new Date().toISOString().slice(0,13)));await page.evaluate(({air,feel,rain,wind,gust,uv})=>{const set=(id,text)=>{const e=document.getElementById(id);if(e)e.textContent=text};set('feels',Number.isFinite(feel)?`${feel.toFixed(1)}°`:'--°');const a=document.getElementById('actual');if(a)a.innerHTML=`Actual <b>${Number.isFinite(air)?air.toFixed(1):'--'}°</b>`;set('range',Number.isFinite(feel)?`Real Feel range ${Math.round(feel-1)}–${Math.round(feel+1)}°`:'Real Feel range --');set('rain',`${Math.round(rain??0)}%`);set('wind',`${Math.round(wind??0)} / ${Math.round(gust??0)}`);set('uv',Number.isFinite(uv)?uv.toFixed(1):'--')},{air:j.current?.temperature_2m,feel:j.current?.apparent_temperature,rain:j.hourly?.precipitation_probability?.[i],wind:j.current?.wind_speed_10m,gust:j.current?.wind_gusts_10m,uv:j.hourly?.uv_index?.[i]})}catch(e){errors.push('Fallback weather injection failed: '+String(e));dataMode='ui-shell-only'}}
  ...
  stage='report';const report=await writeReport();if(report.terminology_hits.length||report.missing_tabs.length||!report.safe_area?.style?.includes('safe-area-inset-top')){fatal='QA assertion failed';process.exitCode=3}
