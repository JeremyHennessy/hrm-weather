...
try{
  ...
  await page.waitForFunction(()=>document.querySelector('#daySummary')?.dataset?.source==='live-current-hourly-summary',{timeout:6000});
  ...
  if(state.summaryOwner!=='live-current-hourly-summary'||!state.summary)throw new Error(`Corrected live-current/hourly summary is not active: owner=${state.summaryOwner||'missing'}`);
  if(state.summaryHourlyCount<1)throw new Error(`Corrected summary did not inspect rendered hourly cards: ${state.summaryHourlyCount}`);
