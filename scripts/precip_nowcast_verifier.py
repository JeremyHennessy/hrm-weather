...
Issue-time forecasts and radar context are archived separately from target-time
truth.  Historical scoring never reuses the collector's current radar state.
Calibration has zero production authority until paired prospective Brier evidence
reaches the minimum sample count and demonstrates non-inferiority to raw.
"""
...
import target_truth_archive as truth_archive
...
def _save(state: dict[str, Any]) -> None:
    state["version"] = VERSION; state["updated_at"] = core.iso(core.utcnow()); cutoff = core.utcnow() - timedelta(days=MAX_AGE_DAYS)
...
def _brier_update(stat: dict[str, Any], probability: float, wet: bool) -> None:
    p=max(0.0,min(1.0,probability/100.0));b=(p-(1.0 if wet else 0.0))**2;n=int(stat.get("n",0));stat["brier"]=(float(stat.get("brier",0.0))*n+b)/(n+1);stat["n"]=n+1;stat["updated_at"]=core.iso(core.utcnow())
...
def _gate(state: dict[str, Any], loc: str, lead: int) -> dict[str, Any]:
    ...
    if n < MIN_GATE_SAMPLES or rb is None or cb is None:
        return {"status":"learning-raw-authoritative","samples":n,"use":"raw","raw_brier":rb,"calibrated_brier":cb,"production_authority":0.0}
    use="calibrated" if cb<=rb else "raw"
    return {"status":"active" if use=="calibrated" else "calibration-disabled-worse-than-raw","samples":n,"use":use,"raw_brier":rb,"calibrated_brier":cb,"relative_improvement":(rb-cb)/rb if rb>1e-9 else None,"production_authority":1.0 if use=="calibrated" else 0.0}


def _family_intensity(forecasts: dict[str, Any], target: datetime) -> float | None:
    fam=v3.current_family_values(forecasts,target,var="precipitation");vals=[core.safe_float(x) for x in fam.values()];vals=[x for x in vals if x is not None];return sum(vals)/len(vals) if vals else None


def _compat_gauge(row:dict[str,Any],obs:dict[str,Any]|None)->dict[str,Any]|None:
    target=core.parse_stamp(row.get("target"));odt=core.parse_stamp((obs or {}).get("time"));gauge=core.safe_float(((obs or {}).get("values") or {}).get("precipitation"))
    if not target or not odt or gauge is None:return None
    offset=abs((odt-target).total_seconds())/60.0
    if offset>TRUTH_MAX_OFFSET_MINUTES:return None
    return {"value":gauge,"valid_time":core.iso(odt),"offset_minutes":offset,"provider":(obs or {}).get("provider") or ("NWS" if row.get("loc")=="uws" else "ECCC"),"source":"latest-official-compat"}


def _actual_from(row:dict[str,Any],obs:dict[str,Any]|None,archive:dict[str,Any]|None)->tuple[float|None,bool|None,dict[str,Any]]:
    target=core.parse_stamp(row.get("target"));loc=str(row.get("loc"));g=truth_archive.nearest_observation(archive or {},loc,target,"precipitation",TRUTH_MAX_OFFSET_MINUTES) if archive else None
    if not g:g=_compat_gauge(row,obs)
    gauge=core.safe_float((g or {}).get("value"));radar_hit=None
    for var in ("radar_rain_rate","radar_extrapolated_rain_rate"):
        radar_hit=truth_archive.nearest_radar(archive or {},loc,target,var,TRUTH_MAX_OFFSET_MINUTES) if archive else None
        if radar_hit:break
    radar=core.safe_float((radar_hit or {}).get("value"));wet=(gauge>=WET_THRESHOLD_MM) if gauge is not None else ((radar>=RADAR_WET_RATE) if radar is not None else None)
    source="official-gauge-target-time" if gauge is not None else ("archived-radar-target-time" if radar is not None else None)
    return gauge,wet,{"source":source,"gauge_mm":gauge,"gauge_valid_time":(g or {}).get("valid_time"),"gauge_offset_minutes":(g or {}).get("offset_minutes"),"radar_rate_mm_h":radar,"radar_valid_time":(radar_hit or {}).get("valid_time"),"radar_offset_minutes":(radar_hit or {}).get("offset_minutes"),"observation_provider":(g or {}).get("provider")}
...
def score_due(state:dict[str,Any],observations:dict[str,Any],archive:dict[str,Any]|None=None)->int:
    ...
    for row in state.setdefault("rows",[]):
        ...
        actual,wet,truth=_actual_from(row,observations.get(row.get("loc")),archive)
...
def score_cycles(state:dict[str,Any],archive:dict[str,Any]|None=None)->int:
    ...
    for (loc,issued),rows in groups.items():
        ...
        for lead in LEADS:
            if predicted[lead]:e["hourly_predicted_wet"]+=1;e["hourly_false_alarms"]+=int(not actual[lead])
        e["onset_errors_h"]=e["onset_errors_h"][-200:];e["cessation_errors_h"]=e["cessation_errors_h"][-200:]
        end_target=core.parse_stamp(by[6].get("target"));rdpa_hit=truth_archive.nearest_radar(archive or {},loc,end_target,"rdpa_6h_precip",TRUTH_MAX_OFFSET_MINUTES) if archive else None;rdpa=core.safe_float((rdpa_hit or {}).get("value"));forecast6=sum(core.safe_float(by[h].get("forecast_intensity_mm_h")) or 0.0 for h in range(1,7))
        if rdpa is not None:_mae_update(state.setdefault("rdpa6",{}).setdefault(loc,{}),forecast6-rdpa)
        state["cycles"].append({"loc":loc,"issued":issued,"actual_onset_lead_h":actual_onset,"predicted_onset_lead_h":pred_onset,"actual_cessation_lead_h":actual_cease,"predicted_cessation_lead_h":pred_cease,"actual_wet":actual,"predicted_wet":predicted,"rdpa_6h_mm":rdpa,"rdpa_truth_valid_time":(rdpa_hit or {}).get("valid_time"),"rdpa_truth_offset_minutes":(rdpa_hit or {}).get("offset_minutes"),"forecast_6h_mm":forecast6,"scored_at":core.iso(core.utcnow())});existing.add((loc,issued));added+=1
    return added
...
def summary(state:dict[str,Any])->dict[str,Any]:
    ...
    return {"version":VERSION,"mode":"prospective-hourly-0-6-target-time-truth","lead_grid_hours":list(LEADS),"wet_threshold_mm":WET_THRESHOLD_MM,"likely_threshold_pct":LIKELY_THRESHOLD_PCT,"target_time_tolerance_minutes":TRUTH_MAX_OFFSET_MINUTES,"calibration_policy":"raw until at least 20 paired prospective Brier samples; calibrated receives production authority only when non-inferior to raw","radar_policy":"issue-time radar is context only; scoring fallback requires radar archived near the forecast valid time","truth_policy":"nearest target-time official gauge within 45 minutes; archived target-time radar occurrence fallback; target-time RDPA retained as separate 6h intensity truth","rows":len(state.get("rows",[])),"scored_rows":sum(1 for r in state.get("rows",[]) if r.get("scored")),"completed_cycles":len(state.get("cycles",[])),"locations":locations,"recent_cycles":(state.get("cycles") or [])[-30:]}


def update(ledger:list[dict[str,Any]],forecasts:dict[str,Any],observations:dict[str,Any],nowcasts:dict[str,Any],archive:dict[str,Any]|None=None)->dict[str,Any]:
    state=_load();score_due(state,observations,archive);score_cycles(state,archive);add_current(state,ledger,forecasts,nowcasts);_save(state);return summary(state)
