...
def cloud_consensus(forecasts:dict[str,Any],target)->dict[str,Any]:
    ...
    return {"available":True,"version":VERSION,"role":"family-aware-cloud-consensus","cloud_cover":mean,"family_spread":spread,"independent_families":len(vals),"sky_condition":classify_cloud(mean),"family_values":families,"calibration_status":"uncalibrated-family-consensus; solar-reference-verification-learning"}
...
def apply(engine:dict[str,Any],forecasts:dict[str,Any])->dict[str,Any]:
    ...
    engine["cloud_sky"]={"version":VERSION,"owner":"accuracy-engine-3-family-cloud-consensus","status":"production-sky-classifier-with-prospective-solar-verification","forecast_points_ready":ready,"thresholds_pct":{"sunny":18,"mostly_sunny":42,"partly_cloudy":68,"mostly_cloudy":88},"wet_weather_policy":"client preserves rain/snow/fog/storm weather types; cloud consensus only resolves dry-sky sunny/partly/cloudy states","verification_policy":"ECCC SWOB shortwave radiation is scored independently; direct observed cloud truth not synthesized","locations":by_location}
    return engine
