/* Final UI truth layer for server-first Engine 3 operation.
   ...
   and prevents a broken live-base zero from overwriting a plausible Engine 3 air temp. */
(()=>{
  ...
  function normalize(){
    ...
    const engineAir=Number(row?.temperature_2m),actual=document.getElementById('actual'),feel=numberFrom(document.getElementById('feels')?.textContent);
    const shownAir=numberFrom(actual?.textContent);
