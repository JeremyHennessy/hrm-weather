/* U.S. presentation layer for the dedicated New York entry point.
   ...
   This file is loaded only when app.html has ?units=us. */
(()=>{
  ...
  const replaceTemps=(text,delta=false)=>String(text||'').replace(/(-?\d+(?:\.\d+)?)\s*°(?:C)?/gi,(_,v)=>`${(delta?deltaCToF(Number(v)):cToF(Number(v))).toFixed(1)}°F`);
  ...
  function tempNode(el,decimals=1){if(!el)return;const raw=el.dataset.wxMetricTemp;if(raw==null){const m=(el.textContent||'').match(/-?\d+(?:\.\d+)?/);if(!m)return;el.dataset.wxMetricTemp=m[0]}const v=n(el.dataset.wxMetricTemp);if(v!=null)set(el,`${cToF(v).toFixed(decimals)}°F`)}
  function actualNode(el){if(!el)return;const text=el.textContent||'',m=text.match(/-?\d+(?:\.\d+)?/);if(!m)return;const current=m[0],stored=el.dataset.wxMetricTemp;if(stored==null||(!text.includes('°F')&&stored!==current))el.dataset.wxMetricTemp=current;const v=n(el.dataset.wxMetricTemp);if(v!=null)el.innerHTML=`Actual <b>${cToF(v).toFixed(1)}°F</b>`}
  function convertCard(root){
    root.querySelectorAll('.zt,.v11DayRF,#hours .hour b').forEach(el=>tempNode(el,1));
    root.querySelectorAll('.sub,.v11DayActual,.v11DayRain,.routineMeta span').forEach(el=>{let t=el.textContent||'';if(!t.includes('°F'))t=replaceTemps(t,false);t=replaceKmh(t);t=replaceMm(t);set(el,t)});
  }
  function paint(){
    ...
    try{
      ...
      for(const id of ['morningActual','eveningActual']){const el=document.getElementById(id);if(el){let t=el.textContent||'';if(!t.includes('°F'))t=replaceTemps(t,false);set(el,t)}}
      const range=document.getElementById('range');if(range){let t=range.textContent||'';if(!t.includes('°F'))t=replaceTemps(t,false);set(range,t)}
      ...
      const total=document.getElementById('rainTotal');if(total){let t=replaceMm(total.textContent||'');set(total,t)}
      const radar=document.getElementById('radarArrival');if(radar)set(radar,replaceMm(radar.textContent||''));
      const summary=document.getElementById('daySummary');if(summary){let t=summary.textContent||'';if(!t.includes('°F'))t=replaceTemps(t,false);t=replaceKmh(t);set(summary,t)}
      const advice=document.getElementById('advice');if(advice){let t=advice.textContent||'';if(!t.includes('°F'))t=replaceTemps(t,false);set(advice,t)}
